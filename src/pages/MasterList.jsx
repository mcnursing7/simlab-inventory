...
export default function MasterList() {
  ...
  const handleScan = scanned => {
    ...
    else { setSearch(scanned); toast(`No match for "${scanned}" — showing search`, {icon:'🔍'}) }
  }

  const handleKey = e => {
    if (e.key==='Enter' && search.trim()) {
      const found = items.find(i => i.sku===search.trim())
      if (found) { setForm({...found, price:found.price+'', min_qty:found.min_qty+'', max_qty:found.max_qty+'', vendor_id:found.vendor_id||''}); setModal(found); setSearch('') }
    }
  }
  ...
  const save = async e => {
    ...
    try {
      ...
      if (modal==='add') await createItem(body); else await updateItem(modal.id, body)
      await reload(); setModal(null); toast.success(modal==='add'?'Item added':'Item updated')
    } catch(e) { toast.error(e.message) } finally { setSaving(false) }
  }
  const del = item => setConfirm({ msg:`Deactivate "${item.name}"?`, onYes:async()=>{ await deleteItem(item.id); await reload(); setConfirm(null); toast.success('Deactivated') } })

  return (
    <div>
      <div style={{display:'flex',gap:8,marginBottom:16,flexWrap:'wrap'}}>
        <div style={{display:'flex',gap:8,flex:1,minWidth:200}}>
          <div className="search-wrap" style={{flex:1}}>{Icons.barcode}
            <input className="search-input" value={search} onChange={e=>setSearch(e.target.value)} onKeyDown={handleKey}
              placeholder="Search or scan SKU…" style={{paddingLeft:32}}/>
          </div>
      ...
      <div className="card"><div className="table-wrap"><table className="dt"><thead><tr>
        ...
        {filtered.map(item=>{
          const qty=totalOnHand(inventory,item.id); const low=qty<=item.min_qty
          return(<tr key={item.id}>
            ...
            <td><div style={{display:'flex',gap:3}}>
              <button className="btn btn-ghost btn-icon btn-sm" onClick={()=>setBcItem(item)}>{Icons.barcode}</button>
        ...
        {filtered.length===0&&<tr><td colSpan={9}><div className="empty">{Icons.package}<p>No items found</p></div></td></tr>}
      </tbody></table></div></div>
      ...
      {modal&&(
        <Modal title={modal==='add'?'Add Item':'Edit Item'} onClose={()=>setModal(null)} size="modal-lg"
          footer={<><button className="btn btn-secondary" onClick={()=>setModal(null)}>Cancel</button><button className="btn btn-primary" onClick={()=>document.getElementById('iform').requestSubmit()} disabled={saving}>{saving?<div className="spin"/>:Icons.check}{modal==='add'?'Add':'Save'}</button></>}>
          <form id="iform" onSubmit={save}>
            <div className="fr2"><Field label="SKU *"><Inp value={form.sku} onChange={f('sku')} required/></Field><Field label="Category *"><Sel value={form.category} onChange={f('category')} required><option value="">Select…</option>{CATS.map(c=><option key={c} value={c}>{c}</option>)}</Sel></Field></div>
            <Field label="Name *"><Inp value={form.name} onChange={f('name')} required/></Field>
            <div className="fr3"><Field label="Unit *"><Inp value={form.unit} onChange={f('unit')} required/></Field><Field label="Price ($) *"><Inp type="number" step="0.01" min="0" value={form.price} onChange={f('price')} required/></Field><Field label="Vendor"><Sel value={form.vendor_id||''} onChange={f('vendor_id')}><option value="">None</option>{vendors.map(v=><option key={v.id} value={v.id}>{v.name}</option>)}</Sel></Field></div>
            <div className="fr2"><Field label="Min Qty *"><Inp type="number" min="0" value={form.min_qty} onChange={f('min_qty')} required/></Field><Field label="Max Qty *"><Inp type="number" min="0" value={form.max_qty} onChange={f('max_qty')} required/></Field></div>
